...
function App() {
  ...
  const CheckmarkIcon = () => (
    <svg className="w-5 h-5 text-neon-blue" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const InstagramIcon = () => (
    <svg className="h-4 w-4 inline-block align-middle mr-1 text-gray-500 hover:text-neon-blue" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 font-orbitron"> {/* Applied font-orbitron class here */}
      <div className="absolute top-6 right-6">
        <button onClick={toggleTheme} className="bg-glass-dark backdrop-blur-glass hover:bg-glass-light text-gray-300 p-3 rounded-xl transition-colors duration-200 shadow-glass text-sm">
          {theme === 'dark' ? 'Light' : 'Dark'}
        </button>
      </div>

      <div className="bg-glass-dark backdrop-blur-glass p-7 rounded-2xl shadow-glass w-full max-w-md flex flex-col">
        <h1 className="text-2xl font-bold text-white absolute top-7 left-7">
          My Tasks
        </h1>

        <div className="mt-14">
          <div className="flex  mb-7 bg-input-bg backdrop-blur-glass rounded-xl px-4 py-1 shadow-input border border-input-border transition-shadow duration-200 focus-within:shadow-glass">
            <input
              type="text"
              className="bg-transparent text-gray-300 flex-grow mr-3 py-1 focus:outline-none placeholder-gray-500 text-sm"
            ...
            />
             <button
              onClick={addTask}
              className="bg-neon-blue text-dark-bg font-bold py-2 px-5 rounded-xl hover:bg-cyan-300 transition-colors duration-200 disabled:opacity-50 shadow-md text-base ml-3 flex-shrink-0"
          ...
          <div className="flex justify-around mb-8 space-x-5">
            {['all', 'pending', 'completed'].map(f => (
              <button
                key={f}
                className={`py-2 px-4 rounded-xl focus:outline-none text-gray-300 hover:text-white transition-colors duration-200 text-base ${filter === f ? 'text-white border-b-2 border-neon-blue rounded-none' : ''}`}
                onClick={() => setFilter(f)}
              >
                {f.toUpperCase()}
              </button>
          ...
          <div className="mb-10">
            {filteredTasks().map(task => (
              <div key={task.id} className="bg-task-bg hover:bg-task-hover rounded-xl mb-3 flex items-center justify-between px-4 py-3 transition-colors duration-200 shadow-sm">
                <button
                  onClick={() => completeTask(task.id)}
                  className="mr-5 focus:outline-none"
                >
                  {task.completed ? <CheckmarkIcon /> : <div className="w-5 h-5 rounded-md border border-gray-500"></div>}
                </button>
                <span className={`${task.completed ? 'line-through opacity-50 text-gray-400' : 'text-gray-300'} text-base flex-grow`}>
                  {task.text}
                </span>
            ...
            {filteredTasks().length === 0 && <p className="text-gray-500 text-center text-base">No tasks here.</p>}
          </div>

          <footer className="text-center text-gray-500 mt-6 pt-4 border-t border-gray-700">
            Made by Osayeed <br/>
